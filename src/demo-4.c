    ...
    while (!WindowShouldClose()) {
        ...
        while (md_ctx_poll(&cmd)) {
            switch (cmd.type) {
                ...
                case MD_COMMAND_DRAW_BOX_OUTLINE: {
                    MDBox box = cmd.as.box_outline.box;
                    MDCorners round = cmd.as.box_outline.round;
                    MDColor col = cmd.as.box_outline.color;
                    f32 thickness = cmd.as.box_outline.thickness;
